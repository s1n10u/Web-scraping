...
broser = webdriver.Chrome(ChromeDriverManager().install())
import pandas as pd
import csv
...
def scraping():
    ...
    browser = webdriver.Chrome(options=options, executable_path=os.path.abspath('chromedriver.exe'))
...
def main():
    ...
    if option == 0:
        ...
        if st.button("DOWNLOAD"):
            df = pd.read_csv("winnistore_data.csv")
            df = df.drop_duplicates(subset=['name', 'price', 'MRP'], keep="first")
            df = df.loc[:, ~df.columns.str.contains('^Unnamed')]

            df.to_csv('winnistore_data.csv')
            csv_downloader(df)
            st.balloons()
        ...
        if st.button("DOWNLOAD_FILE"):
            df = pd.read_csv("manakiranam_data.csv")
            df = df.drop_duplicates(subset=['name', 'price', 'MRP'], keep="first")
            df = df.loc[:, ~df.columns.str.contains('^Unnamed')]

            df.to_csv('manakiranam_data.csv')
            csv_downloader(df)
            st.balloons()
