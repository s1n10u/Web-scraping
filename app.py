...
def scraping():
    ...
    data.to_csv('C:/Users/suresh/PycharmProjects/pythonProject/jiostore_data.csv')
    data = data
...
def main():
    ...
    if option == 0:
        ...
        if st.button("DATA_DOWNLOAD"):
            df = pd.read_csv("C:/Users/suresh/PycharmProjects/pythonProject/jiostore_data.csv")
            df = df.drop_duplicates(subset=['name', 'price', 'MRP'], keep="first")
            df = df.loc[:, ~df.columns.str.contains('^Unnamed')]

            df.to_csv('C:/Users/suresh/PycharmProjects/pythonProject/jiostore_data.csv')
            csv_downloader(df)
            st.balloons()
        ...
        if st.button("DOWNLOAD"):
            df = pd.read_csv("D:/Users/sanjana/PycharmProject/Gharwaalaa/winnistore_data.csv")
            df = df.drop_duplicates(subset=['name', 'price', 'MRP'], keep="first")
            df = df.loc[:, ~df.columns.str.contains('^Unnamed')]
        ...
        if st.button("DOWNLOAD_FILE"):
            df = pd.read_csv("D:/Users/sanjana/PycharmProject/Gharwaalaa/manakiranam_data.csv")
            df = df.drop_duplicates(subset=['name', 'price', 'MRP'], keep="first")
            df = df.loc[:, ~df.columns.str.contains('^Unnamed')]
